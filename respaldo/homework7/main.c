...
int main()
{
    ...
    while(salir==0){
        ...
        printf("Este programa usa la funcion scanf, por favor cuide que su entrada es correcta antes de dar enter\n");
...
void registrar(){
    ...
    do {
        printf("\nEscriba a que hora y minuto trabajara\n");
        scanf("%d:%d", &hora, &min);
    } while(hora<0||hora>23||min<0||min>60);
        
    do {
        printf("\nCuantos segundos trabajara\n");
        scanf("%d", &trabajo);
...
void horario(){
    ...
    FILE* escribirhorario = fopen("sistema.txt", "w");
    char c;
    int count;
    ...
    rewind(leersistema); // regresamos al inicio
    for (i = 0; i < count; i++) {
    fscanf(leersistema, "%d %s %d %d %d %d %d %d %d %d %d %d", ); 
