...
void delete(node* position, char* u, char* p, int* n){/////// A la funcion eliminar le damos el usuario y la contrase
    ...
    if (position != NULL) {
        ...
        if (strcmp(position->user, u) == 0&&strcmp(position->password, p) == 0){
            ///si no tiene hijos lo eliminamos completamente
            if (position->leftChild == NULL && position->rightChild == NULL) { // no children
            free(position);
            printf("Usuario %s eliminado No tiene hijos\n\n", u);
            *n = 1;
            }
            else if (position->leftChild != NULL || position->rightChild != NULL&&a==0){ ///Si tiene hijos 
            position->password=NULL;
            printf("Usuario %s eliminado Tiene hijos\n\n", u);
            *n = 1;
            }
        }
...
int main() {
  ...
  while ((l = saca()) == 0) {
    ...
    else if (strstr(linea, "show") != NULL&&strlen(linea)==4) { // if we found  "show" we need to show
      show(tree, '+');
      printf("\n\n");
      contadorespacios=0;
    }
...
void show(node* position, char prefix) {
  ...
  if (position != NULL) {
    ...
    printf("%c %s (%s) %c\n", prefix, position->user, position->password, suffix);
