/* 
    ...
    File organization 
    ----------------------
    counting for statistics.txt we have the diseases to count and add to the graphs
    [asthma]
    [COVID-19]
    [fever]
    [flu] 
    ----------------------------------------
    graphstatics we see a graph of the diseases that we want
    ...
    -------------------
    vitalsigns
    information to use in a doctor appointment
    -------------------
    textstatics 
    same information of the graph but as a text
    -------------
    name.txt
    example-> Angel Raul Chavez Carrillo.txt
    general information about the doctor appointment
    -------------------
    date.txt
    information about the next doctor appointment
    Name 
    Hour
    Day
    Doctor's office number: 123
    Reason: 
    -------------------
...
void erase(node* pos) { // This part is from the repository of SatuElisa https://github.com/satuelisa/C/blob/main/Ch6/wordcount.c 
  if (pos != NULL) { //If the position is not NULL we can eliminate 
    erase(pos->izq);
    erase(pos->der);
    free(pos->word);
    free(pos);
...
// In orden to learn a litle bit of Binary trees i read https://www.geeksforgeeks.org/binary-search-tree-set-2-delete/
void set_depth(node* pos, int d) { 
// This part is from the repository of SatuElisa https://github.com/satuelisa/C/blob/main/Ch6/wordcount.c  
// set the depth of the tree
  if (pos != NULL) { //if the position is not NUll
    pos->depth = d;
#ifdef DEBUG
...
// insert a node to the tree
  int cmp;
  if (pos == NULL) {
#ifdef DEBUG //This parts is to see the work of the parts that use the nodes 
...
#endif
    cmp = strcmp(pos->word, word); // We "compare" two strings if is 0 are equal 
    //https://www.aprendeaprogramar.com/referencia/view.php?f=strcmp&leng=c I read about it 
    if (cmp == 0) {
#ifdef DEBUG
...
void alpha(node* pos) { //A litle modification of the file wordcount.c from satuelisa 
  FILE* textstatics = fopen("textstatistics.txt", "a"); // here only text
  FILE* graphstatics = fopen("graphstatistics.txt", "a"); // here only text
  int total_patients;
  int number=20; //number of * that are the 100% 
  int limit;
  /*
  In order to explain how i do the graph
  lets say that 100 % are 20 *
  
  so 
   20 * = 100 %
        = pos->counter/total_patients * 100 (this will be the % )
        
   we use a rule of 3 simple 
   
   [(pos->counter/total_patients * 100) * 20]/100 

   we cancel the 100
   
    [(pos->counter/total_patients) * 20]
...
int main()
{
    ...
    printf("\n");
    // Open some files to use later....
    FILE* general = fopen("counting for statistics.txt", "a");
    fclose(general);
    FILE* registered = fopen("registered.txt", "a");
    fclose(registered);
    header(); // Only print a header
    char specificname[MAX]; // Array to save the name of a file that you want to read 
    configuration(); // Do the file vitalsigns
    ...
    while(end==FALSE){ 
        ...
        if (option==1){ // Set a date for a doctors appointment
            ...
            FILE* textstatics = fopen("textstatistics.txt", "w"); // Some files to use later 
            fclose(textstatics);
...
int count(char* file){//count lines in a file 
    // Modification of  https://github.com/satuelisa/C/blob/main/Ch7/archivo.c
...
int printfile(char* file){ //print a txt file in console
    FILE* sistema = fopen(file, "r"); // Read the file 
    char line[MAX]; // we print the file in lines
    char c;
    int i;
    i=0;
    while ((c = fgetc(sistema)) != EOF) {
        if (c == '\n') {
            line[i]='\0'; // we add the end character
            printf("%s\n", line);
            i=0;
        }
        else{
            line[i]=c; //if is not a '\n' we save it 
            i++;
        }
    }
    printf("\n");
...
void date(){
    ...
    write("date.txt"); // Function to write a lot of text in the filr
    printf("\n");
    }
}
...
void doctor(){ // Doctors appointment
    ...
    */
    ...
    write(filename); //write a lot of text 
    printf("\n");
    ...
    write(filename); //write a lot of text  
    printf("\n");
    char disease[MAX]; //name of the disease that you want to add to statistics
    printf("Do you want to add some disease to statistics\nWrite a single X->No // or write the disease\n");
    do {
        saca(disease); //obtain the disease 
        if (disease[0]!='X' && strlen(disease)!=1){ //if the string is not a "X"
            ...
            fprintf(general, "[%s]\n", disease); // print the disease in the file counting for statistics.txt
            fclose(general); //close the file 
        }
