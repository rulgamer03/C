/*
Usar junto con el archivo extenciones.txt
*/
#include <stdio.h>
#include <string.h>
#define MAX 99
...
int verificarcorreo(char* correo) //return 1 si el correo es valido // return 0 si no es correo valido
{
    ...
    extencion[j]='\0';
    FILE* entrada = fopen("extenciones.txt", "r");
    char extencionesdisponibles[MAX];
    while (fscanf(entrada, "%s", extencionesdisponibles) == 1) {
    if (strcmp(extencion, extencionesdisponibles) == 0&&longitude>=strlen(extencionesdisponibles)+LIMIT) { 
